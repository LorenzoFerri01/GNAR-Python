...
class TestIntervalsAndTests:
    ...
    def test_beta_at(self, fit):
        ...
        # At the w-weighted mean log degree (N0="decorrelated"), beta_0 is uncorrelated with kappa_hat
        _, log_N, w = degree_terms(fit.degrees, fit.kappa)
        wi = w * w * fit._R.d
        N_star = np.exp(np.sum(wi * log_N) / np.sum(wi))
        assert fit.beta_at("decorrelated")["N0"] == pytest.approx(N_star, rel=1e-12)
...
class TestVarianceBoundValues:
    """The bounds' values, not only that they contain the exact variances (acceptance test 9 checks containment)."""

    @pytest.mark.parametrize("graph", list(BOUND_GRAPHS))
    @pytest.mark.parametrize("kappa", [0.0, 0.3, 0.7, 1.2])
    def test_upper_bounds_attained_without_dependence(self, graph, kappa):
        # With alpha = 0 and beta -> 0 the series are white noise: v_i = N_i and Gamma0 = I, so every upper bound is attained
        A = BOUND_GRAPHS[graph]
        d = A.shape[0]
        var = np.diag(asymptotic_cov(A, 0.0, 1e-4, kappa))
        bounds = variance_bounds(A, 0.0, 1e-4, kappa)
        assert var[d + 1] / bounds["kappa"]["upper"] == pytest.approx(1, rel=1e-6)
        assert var[d] / bounds["beta"]["upper"] == pytest.approx(1, rel=1e-6)
        np.testing.assert_allclose(var[:d] / bounds["alpha"]["upper"], 1, rtol=1e-6)

    def test_star_hand_formulas(self):
        # Star with m leaves: log N is L = log m at the hub and 0 at the leaves, so for g_i = N_i^p (hub g_h = m^p)
        # S_kappa(g) = g_h L^2 m / (g_h + m) and S_beta(g) = m
        m, kappa = 8, 0.6
        A = star_graph(m)
        ...
        def S_kappa(p):
            g_h = m ** p
            return g_h * L ** 2 * m / (g_h + m)

        gamma_bar = bounds["gamma_bar"]
        gamma_rho = 1 / (1 - bounds["rho"] ** 2)
        assert bounds["kappa"]["upper"] == pytest.approx(1 / (beta ** 2 * S_kappa(1 - 2 * kappa)), rel=1e-12)
        assert bounds["kappa"]["lower"] == pytest.approx(1 / (beta ** 2 * gamma_bar * S_kappa(2 - 2 * kappa)), rel=1e-12)
        assert bounds["kappa"]["lower_rho"] == pytest.approx(1 / (beta ** 2 * gamma_rho * S_kappa(2 - 2 * kappa)), rel=1e-12)
        assert bounds["beta"]["upper"] == pytest.approx(1 / m, rel=1e-12)
        assert bounds["beta"]["lower"] == pytest.approx(1 / (gamma_bar * m), rel=1e-12)
        assert bounds["beta"]["lower_rho"] == pytest.approx(1 / (gamma_rho * m), rel=1e-12)

    def test_alpha_bounds_and_scaling(self):
        from gnar.utils.simulating import stationary_cov, row_sum_bound
        A = BOUND_GRAPHS["er"]
        rng = np.random.default_rng(82)
        alpha, beta = draw_params(A, 0.8, rng)
        per_period = variance_bounds(A, alpha, beta, 0.8)
        rho = row_sum_bound(A, alpha, beta, 0.8)
        np.testing.assert_allclose(per_period["alpha"]["lower"], 1 / np.diag(stationary_cov(A, alpha, beta, 0.8)), rtol=1e-12)
        np.testing.assert_allclose(per_period["alpha"]["lower_rho"], 1 - rho ** 2, rtol=1e-12)
        np.testing.assert_array_equal(per_period["alpha"]["upper"], 1.0)
        assert per_period["gamma_bar"] == pytest.approx(np.max(np.diag(stationary_cov(A, alpha, beta, 0.8))), rel=1e-12)
        # With n = T every bound is the per-period bound divided by T - 1
        T = 501
        scaled = variance_bounds(A, alpha, beta, 0.8, n=T)
        for name in ("kappa", "beta", "alpha"):
            for key in ("lower", "lower_rho", "upper"):
                np.testing.assert_allclose(scaled[name][key], np.asarray(per_period[name][key]) / (T - 1), rtol=1e-12)


class TestTheoryAtBetaZero:

    def test_non_regular_at_beta_zero(self):
        # kappa does not enter the model when beta = 0: Var(kappa_hat) is infinite and no information-matrix variance
        # applies to the others; asymptotic_cov and closed_form_variances agree
        A = star_graph(5)
        with pytest.warns(UserWarning, match="not identified"):
            cov = asymptotic_cov(A, 0.3, 0.0, 1.0)
        assert np.isinf(cov[-1, -1]) and np.isnan(cov[-2, -2])
        with pytest.warns(UserWarning, match="not identified"):
            closed = closed_form_variances(A, 0.3, 0.0, 1.0)
        assert np.isinf(closed["kappa"]) and np.isnan(closed["beta"])
        # On a graph where every node has degree 1, N_i^(-kappa) = 1 and beta is regular even at beta = 0
        B = np.kron(np.eye(3), np.array([[0.0, 1.0], [1.0, 0.0]]))
        with pytest.warns(UserWarning):
            cov_b = asymptotic_cov(B, 0.3, 0.0, 1.0)
        assert np.isfinite(cov_b[-2, -2]) and np.isinf(cov_b[-1, -1])


class TestLevelsAndNullValues:

    def test_non_default_level(self, fit):
        d = fit.d
        table = fit.ci(level=0.9)
        wald = fit.ci(level=0.9, method="wald")
        z = norm.ppf(0.95)
        assert wald.loc["beta", "lower"] == pytest.approx(fit.beta - z * fit.se[d], rel=1e-12)
        assert wald.loc["kappa", "upper"] == pytest.approx(fit.kappa + z * fit.se[d + 1], rel=1e-12)
        for end in ("lower", "upper"):
            k = table.loc["kappa", end]
            dev = deviance(fixed_kappa_ols(fit._M, fit.degrees, k, fit._R)[2], fit.rss, fit.n_obs)
            assert dev == pytest.approx(chi2.ppf(0.9, 1), abs=1e-8)

    def test_other_null_value(self, fit):
        test = fit.test_kappa(0.5)
        assert test["z"] == pytest.approx((fit.kappa - 0.5) / fit.se[-1], rel=1e-12)
        rss_05 = fixed_kappa_ols(fit._M, fit.degrees, 0.5, fit._R)[2]
        assert test["lr"] == pytest.approx(fit.n_obs * np.log(rss_05 / fit.rss), rel=1e-12)

    def test_null_outside_bounds(self):
        # The LR test compares k0 with the maximum over kappa_bounds, so it is not available for k0 outside them
        A = random_tree(10, seed=11)
        X, _, _ = simulate(A, 1.0, 2000, seed=12)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", KappaWarning)
            fit = fit_gnar1(A, X, kappa_bounds=(0, 0.5), demean=False)
        test = fit.test_kappa(1.0)
        assert np.isfinite(test["z"]) and np.isnan(test["lr"]) and np.isnan(test["lr_p_value"])
        assert "LR not available" in str(fit)


class TestFixedSigmaInference:

    def test_intervals_and_lr_use_the_fixed_variance(self):
        # With sigma^2 fixed far from RSS / n, the profile deviance (RSS(kappa) - RSS(kappa_hat)) / sigma^2 differs from
        # n log(RSS(kappa) / RSS(kappa_hat)), so intervals and tests that ignored sigma_2 would be visibly wrong
        A = random_tree(10, seed=71)
        X, _, _ = simulate(A, 0.7, 2000, seed=72)
        fit = fit_gnar1(A, X, sigma_2=4.0, demean=False)
        interval = fit.profile_interval()
        for end in ("lower", "upper"):
            rss = fixed_kappa_ols(fit._M, fit.degrees, interval[end], fit._R)[2]
            assert deviance(rss, fit.rss, fit.n_obs, 4.0) == pytest.approx(chi2.ppf(0.95, 1), abs=1e-8)
        rss_1 = fixed_kappa_ols(fit._M, fit.degrees, 1.0, fit._R)[2]
        assert fit.test_kappa()["lr"] == pytest.approx((rss_1 - fit.rss) / 4.0, rel=1e-12)
        # The standard errors use the fixed sigma^2 too
        estimated = fit_gnar1(A, X, demean=False)
        assert fit.se[-1] == pytest.approx(estimated.se[-1] * np.sqrt(4.0 / estimated.sigma_2), rel=1e-10)


class TestProfileIntervalShape:

    def test_disconnected_confidence_set(self):
        from gnar.utils.gnar_profile_likelihood import NodeSums, residualised_sums, estimate_kappa
        from gnar.utils.gnar_inference import profile_ci
        # Two groups of nodes pulling kappa in opposite directions give a bimodal profile: the confidence set is
        # [0, 0.585] joined with [1.41, 1.5]
        M = NodeSums(xx=np.ones(4), xy=np.zeros(4), xs=np.zeros(4), ss=np.array([1.0, 1.0, 100.0, 100.0]),
                     sy=np.array([2.0, 2.0, -20.0, -20.0]), yy=np.full(4, 1e5), n=1000)
        degrees = np.array([1.0, 1.0, 10.0, 10.0])
        R = residualised_sums(M)
        est = estimate_kappa(M, degrees, (0.0, 1.5), 201, sigma_2=1.0, R=R)
        interval = profile_ci(M, R, degrees, est.kappa, est.rss, est.grid, 3996, sigma_2=1.0)
        assert interval["disconnected"] and interval["lower_open"] and not interval["upper_open"]

    def test_connected_interval_has_no_note(self, fit):
        assert not fit.profile_interval()["disconnected"]
        assert fit.ci().loc["kappa", "note"] == ""


class TestBetaAtConsistency:

    def test_same_before_and_after_cov(self):
        A = random_tree(10, seed=71)
        X, _, _ = simulate(A, 0.7, 2000, seed=72)
        fit = fit_gnar1(A, X, demean=False)
        before = fit.beta_at()
        _ = fit.cov
        assert fit.beta_at() == before

    def test_no_network_information(self):
        X = np.random.default_rng(0).standard_normal((50, 3))
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", KappaWarning)
            fits = [fit_gnar1(np.zeros((3, 3)), X, kappa=k, demean=False) for k in (1.0, None)]
        for f in fits:
            ref = f.beta_at(N0=2.0)
            assert ref["beta_0"] == 0.0 and np.isnan(ref["se"])
            _ = f.cov
            ref_after = f.beta_at(N0=2.0)
            assert ref_after["beta_0"] == 0.0 and np.isnan(ref_after["se"])
            assert np.isnan(f.ci().loc["beta_0", "se"])

    def test_decorrelated_needs_identified_kappa(self):
        X = np.random.default_rng(0).standard_normal((50, 3))
        fixed = fit_gnar1(path_graph(3), X, kappa=1.0, demean=False)
        with pytest.raises(ValueError, match="decorrelated"):
            fixed.beta_at("decorrelated")
        with pytest.raises(ValueError, match="N0"):
            fixed.beta_at("median")
