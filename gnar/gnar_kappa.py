...
class GNARKappaFit:
    ...
    Attributes:
        ...
        sigma_2 (float): Noise variance: RSS / (n_obs - p) with p = d + 2 (kappa estimated) or d + 1 (kappa fixed), or
            the fixed value passed to fit_gnar1.
...
def fit_gnar1(
    ...
    *,
    ...
    p = d + 2 (kappa estimated) or d + 1 (kappa fixed), unless sigma_2 is given.
    ...
    p = d + 1 + int(kappa_estimated)
    if sigma_2 is None and n_obs <= p:
        raise ValueError(f"Too few observations ({n_obs}) for {p} parameters.")
